...
const storage = multer.diskStorage({
    destination: async (req, _file, cb) => {
        try {
            const { momentId, categoryKey } = req.params;
...
app.post('/api/moments', async (req, res) => {
    ...
    let existing = null;
    if (id)
        existing = await getMomentById(id);
    const fullCategories = Object.fromEntries(CATEGORY_KEYS.map((key) => [
        key,
        {
            notes: categories?.[key]?.notes ?? (existing?.categories[key]?.notes ?? ''),
...
app.delete('/api/moments/:momentId/categories/:categoryKey/images/:imageId', async (req, res) => {
    try {
        const { momentId, categoryKey, imageId } = req.params;
        const moment = await getMomentById(momentId);
        if (!moment)
            return res.status(404).json({ error: 'Moment not found' });
        if (!CATEGORY_KEYS.includes(categoryKey))

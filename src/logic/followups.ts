import type { FollowUpQuestion, Moment } from '../types.js';
import { v4 as uuid } from 'uuid';

export function generateFollowUps(previous: Moment | null, current: Moment): FollowUpQuestion[] {
  const followUps: FollowUpQuestion[] = [];

  for (const [category, entry] of Object.entries(current.categories)) {
    const prevNotes = previous?.categories?.[category as keyof typeof previous.categories]?.notes ?? '';
